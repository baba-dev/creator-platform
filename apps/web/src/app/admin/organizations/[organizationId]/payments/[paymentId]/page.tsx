...
export default async function PaymentDetailPage({
  ...
  const [ledgerEntry, wallet] = await Promise.all([
    payment.ledgerEntryId
      ? db.ledgerEntry.findUnique({ where: { id: payment.ledgerEntryId } })
      : null,
          ...
          <dl className="mt-4 divide-y divide-border text-sm">
            ...
            <div className="flex justify-between py-3">
              ...
              <dd className="tabular-nums text-foreground">
                {payment.receivedAt.toLocaleDateString("en-OM", {
                  year: "numeric",
                  month: "long",
                  day: "numeric",
                })}
            ...
            {payment.confirmedAt && (
              <div className="flex justify-between py-3">
                ...
                <dd className="tabular-nums text-foreground">
                  {payment.confirmedAt.toLocaleString("en-OM")}
                </dd>
              </div>
            )}
            {payment.confirmedBy && (
            ...
            <div className="rounded-2xl border border-border bg-card p-6">
              ...
              <dl className="mt-4 divide-y divide-border text-sm">
                ...
                <div className="flex justify-between py-2">
                  ...
                  <dd className="tabular-nums font-semibold text-foreground">
                    +{formatCredits(ledgerEntry.amountCredits)}
                  </dd>
                </div>
                <div className="flex justify-between py-2">
                  <dt className="text-muted-foreground">Balance after</dt>
                  <dd className="tabular-nums text-foreground">
                    {formatCredits(ledgerEntry.balanceAfter)}
                  </dd>
                </div>
                <div className="flex justify-between py-2">
                  <dt className="text-muted-foreground">Timestamp</dt>
                  <dd className="tabular-nums text-muted-foreground">
                    {ledgerEntry.createdAt.toLocaleString("en-OM")}
                  </dd>
                </div>
              </dl>
            </div>
